...
**State as of 2026-09-22 (09:30).** Best public LB **0.939** — submission #12, the **P-27 fork**: the public 0.942 notebook's inference graph verbatim (cells 0–49, ~35 public checkpoints, trains nothing) with our c02 members rank-blended on top at β 0.20. That is 0.003 *under* the anchor's author-stated 0.942, so the fork has not yet shown a gain from our arm; as evidence −0.003 is 🔁 (under the 0.005 LB floor). **Our own blend alone is 0.913** (#11: two DINOv2 heads + five concat folds + one ConvNeXt-T + the three c02 window-attention members `v08w` / `v10c` / `v09h`, one vote per version). **Scoring now:** #13 = the fork at β 0.10 (ref 56458837) and #14 = β 0.10 with the two **P-28 production members** `v09a` (CoAtNet-1, all 4,349 studies, 16 ep, SWA; gold-58 0.8768) and `v08a` (DINOv2-S, 0.8816) added to our arm (ref 56459131); read-out rules in the handoff's in-flight table. **Fork v6 = the anchor-only control (β 0, our arm not run)** is pushed and, once its placeholder run is green, ready to submit on Tian's go — it fixes the anchor's score from *our* account, which every β read is relative to. Public LB top is **0.958**. Kaggle: 2 GPU sessions, 5 submissions/day, OAuth tokens last ~3 h (traps 20). Full session history, in-flight items and next actions: [docs/handoff.md](docs/handoff.md). This file keeps only the current state: the fifteen dated "State as of" paragraphs that accumulated here (2026-08-29 → 2026-09-22) duplicated the handoff's session entries and were removed on 2026-09-22; the LB progression 0.500 → 0.841 → 0.871 → 0.877 → 0.896 → 0.900 → 0.909 → 0.912 → 0.913 → 0.939 is the Scoreboard in [docs/experiments.md](docs/experiments.md).
...
| [docs/proposals.md](docs/proposals.md) | **Ranked backlog as testable cards P-00…P-28** (hypothesis, evidence, measure, noise floor, cost) | When choosing what to do next |
