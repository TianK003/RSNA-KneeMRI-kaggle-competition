...
**State as of 2026-09-21 (21:10):** public LB top is **0.958**; we are **0.913**. `notebook_score_0.942.ipynb` (public 0.942) was read cell by cell: **it trains nothing** — ~35 public checkpoints (20 DINOv2, A5 16-slice, RadImageNet + calibrator, Raptor CoAtNet-2@384 ×4 @94 windows, a CoAt family) rank-fused with LB-tuned weights; its FineSpacing stage was never attached in the scored run. **Tian's decisions:** fork it verbatim and add our members as one more vote (**P-27**, `src/build_fork.py` → `kaggle/rsna-knee-fork/`, deterministic; fork v1 proved every stage runs on Kaggle, v3 = the fixed arm cell, **not yet submitted**), AND keep training our own under the **P-28 production regime** (all 4,349 studies, 16 ep, SWA of the last 3 EMA snapshots; `train_all` / `swa_last` / `ARM_ONLY`; arms `v09a` CoAtNet-1 c02 — **training, `rsna-knee-train` v19** — and `v08a` DINOv2 c02, pushed when a GPU slot frees). Licences of the public checkpoints read: all CC0 except the three RadImageNet datasets (CC-BY-NC-SA / "other"). Kaggle allows 2 GPU sessions; 5 submissions/day. Next: fork v3 green → submit #12 (read-out ≥ 0.947 real / 0.940–0.946 🔁 / < 0.940 hurts); morning: ship `v09a`/`v08a` → `build_fork.py --members …` → #13; RunPod attended for ConvNeXt-T c02 PROD. See [docs/handoff.md](docs/handoff.md).
