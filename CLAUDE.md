...
**State as of 2026-09-23 (08:50).** Best public LB **0.942** — tied by #13 (the P-27 fork: the public 0.942 notebook's inference graph verbatim + our c02 members at β 0.10) and #15 (the anchor alone, our arm not run). **The anchor reproduces 0.942 from our account; our arm is worth ±0.000 at β 0.10, −0.003 at β 0.20 (#12, 0.939), −0.001 with the 16-epoch P-28 members added (#14, 0.941), and the flat-0.60 outer-map hedge (#16, arm not run) read 0.940** — every delta under the 0.005 floor (experiments.md 2026-09-22 P-27 read-out; 2026-09-23 "#16 … why nothing of ours moves 0.942"). **Why nothing of ours moves 0.942:** the anchor is already a superset of the best public notebook (Speedy Raptors 0.943; the "0.957" in a public title is a gold-58 number — traps 35), so re-anchoring is not a lever; our members are 0.86–0.87 OOF (≈ 0.89–0.90 solo on the LB by the measured +0.02–0.03 offset) added at β 0.10 to a stack whose members are 0.90–0.928 solo, and below ~0.90 solo a correlated member costs more than its diversity buys; 690 teams sit at 0.941–0.942 and everything ≥ 0.945 is private work. **P-29:** the 16-epoch production schedule over-trains (fold-0 OOF peaks at epoch 8) → `PROD` trains 8 epochs. **S1 A/B (`rsna-knee-train` v23, read 2026-09-23; both T4s via `PARALLEL_ARMS`, P-31 ✅ two arms in 3.22 h):** `v09b` (two-study BatchNorm batches, P-32) **0.8690** and `v09c` (+ light augmentation, P-33) **0.8730** vs `v09h` 0.8683 — both 🔁 under the 0.008 floor, same direction, so both knobs ride in the S2 production `v09a` (experiments.md 2026-09-23 "S1 A/B"). **S2 done (`rsna-knee-train` v26, 2026-09-23, 2.64 h for both arms on the two T4s):** `v09a` (8 ep, batch 2, aug light) gold-58 SWA **0.8922** (16-ep: 0.8768) and `v08a` (8 ep) **0.8850** (0.8816), both still rising at epoch 7 — the 8-epoch regime is the production recipe; gold deltas are direction only. Shipped as new versions of `rsna-knee-ckpt-v09a` / `-v08a`; **submitted in the fork at β 0.10 as #17** (fork v8, four members) vs 0.942 (floor 0.005; ≥ 0.947 = our arm counts; honest expectation 0.942 ± 0.001). **Round 2 is staged, smoke-green, awaiting a go:** `v09d` (CoAtNet backbone LR 3e-5 on the `v09c` recipe, P-34) ‖ `v08c` (`v08w` + light aug, P-35) in `rsna-knee-folds` (`artifacts/folds_r2_real.py`, ≈ 3.3 h, both T4s). A `kaggle kernels push` that errors with `Expecting value` may still have created the version — check `kernels status` before retrying (traps 36). Our own blend alone is 0.913 (#11). Public LB top is **0.958**. Kaggle: 2 GPU sessions, 5 submissions/day, OAuth tokens last ~3 h (traps 20). Session history, in-flight items and next actions: [docs/handoff.md](docs/handoff.md); the LB progression 0.500 → … → 0.913 → 0.939 → 0.942 is the Scoreboard in [docs/experiments.md](docs/experiments.md).
