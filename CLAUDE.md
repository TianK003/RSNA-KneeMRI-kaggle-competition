...
**State as of 2026-09-22 (08:10):** **Submission #12 (the P-27 fork, β 0.20) scored 0.939** — best number on the board, but 0.003 *under* the anchor's own 0.942: the pre-registered action rule fires (next: β 0.10 = fork v4, built; then an anchor-only β = 0 control, because the anchor was never scored from our account and the arm is fail-soft), while as evidence −0.003 is 🔁 (experiments.md Submissions #12). **Both P-28 production arms trained** (`v09a` CoAtNet-1 5.1 h, gold-58 SWA 0.8768; `v08a` DINOv2-S 2.4 h, 0.8816; SWA ≈ last EMA; both gold curves peak at epoch 5–6 and drift ≈ −0.015 by epoch 15 — inside the gold SE, logged as an open question). `v09a` checkpoints pulled to `artifacts/kaggle_out/train_v19/` and staged in `artifacts/ship_v09a/`; the `v08a` pull hit the expired OAuth token (traps 20: `kaggle auth login --force`). Next: login → ship `rsna-knee-ckpt-v09a` / `-v08a` → push fork v4 (β 0.10) → submit #13 → fork v5 (`--members v08w v09h v09a v08a`, β 0.10) → #14; anchor-only control on Tian's call. See [docs/handoff.md](docs/handoff.md).
**State as of 2026-09-21 (21:10):** public LB top is **0.958**; we are **0.913**. `notebook_score_0.942.ipynb` (public 0.942) was read cell by cell: **it trains nothing** — ~35 public checkpoints (20 DINOv2, A5 16-slice, RadImageNet + calibrator, Raptor CoAtNet-2@384 ×4 @94 windows, a CoAt family) rank-fused with LB-tuned weights; its FineSpacing stage was never attached in the scored run. **Tian's decisions:** fork it verbatim and add our members as one more vote (**P-27**, `src/build_fork.py` → `kaggle/rsna-knee-fork/`, deterministic; fork v1 proved every stage runs on Kaggle, **v3 green and sent as submission #12 at 21:24 (ref 56442019, scoring ≈ 5–7 h; read-out ≥ 0.947 real / 0.940–0.946 🔁 / < 0.940 hurts)**), AND keep training our own under the **P-28 production regime** (all 4,349 studies, 16 ep, SWA of the last 3 EMA snapshots; `train_all` / `swa_last` / `ARM_ONLY`; arms `v09a` CoAtNet-1 c02 — **training, `rsna-knee-train` v19** — and `v08a` DINOv2 c02 — **training, `rsna-knee-folds` v6**). Licences of the public checkpoints read: all CC0 except the three RadImageNet datasets (CC-BY-NC-SA / "other"). Kaggle allows 2 GPU sessions; 5 submissions/day. Next: fork v3 green → submit #12 (read-out ≥ 0.947 real / 0.940–0.946 🔁 / < 0.940 hurts); morning: ship `v09a`/`v08a` → `build_fork.py --members …` → #13; RunPod attended for ConvNeXt-T c02 PROD. See [docs/handoff.md](docs/handoff.md).
**State as of 2026-09-03:** ⭐ **Best public LB 0.913 — submission #11 read** (infer v14 = the seven-version blend with `v09h` as 5 folds; +0.001 over #10 is **🔁 exactly as pre-registered** — folds are replicates inside one vote of seven; it stays the default blend because it is the best number and the more robust one privately; offset +0.031, the first point outside +0.020–0.030, for the #6 reason: a fold-0 proxy cannot price fold averaging). **Nothing is in flight** — no kernels, no pods, no watchers; GPU quota should be a fresh week (last run 2026-08-30). **Tian moved to a new laptop and the workspace is rebuilt and verified** (`.venv` on Python **3.11**, so `numpy==2.5.2` / `torch==2.13.0` are off-pin at 2.4.6 / 2.14.0+cpu — every other pin exact; `build_targets.py` still prints 0.8948 / folds 882/882/881/881/881, `cache_selftest.py` + `window_head_test.py` green with all four backbones offline in `models/`, local train-mode smoke green). New footgun: on a fresh machine the local smoke needs `MODE="train"` sed'd in, because the copied `artifacts/kaggle_out` checkpoints flip `"auto"` into infer (traps 30). Next GPU work is unchanged: a new **input representation** (P-23 #3/#4) or P-17 self-training. See [docs/handoff.md](docs/handoff.md).
**State as of 2026-08-30 (22:40):** **Submission #11 sent 21:55 (infer v14 = the 0.912 blend with `v09h` as 5 folds; ref 55899052) — ⏳ scoring ≈ 2 h 40 min; expected 0.912–0.916 (🔁 by design), ≥ 0.917 = real.** 1 submission left today. The 22:15 state follows.
...
kaggle/rsna-knee-infer/     MODE="infer" copy of OUR blend alone (submissions #1-#11)
