...
| 2026-09-22 | **P-27 fork, submission #13 (`rsna-knee-fork` v4, sent 08:30, ref 56458837)**: #12's graph and members (`v08w` + 5-fold `v09h`) at **β = 0.10** — the pre-registered follow-up to 0.939; placeholder green (`beta0.10`, rc 0, 98 s) | — | ⏳ scoring ≈ 6–10 h | read vs #12 (0.939) and the anchor's 0.942: ≥ 0.945 = the arm helps at low β; 0.940–0.944 🔁; ≤ 0.939 = β is not the lever (the anchor-only control decides between "arm hurts" and "anchor drifted") |
| 2026-09-22 | **P-27 + P-28 fork, submission #14 (`rsna-knee-fork` v5, sent 08:41, ref 56459131)**: #13's graph + the two all-data SWA members — arm = `v08w` + 5-fold `v09h` + **`v09a` + `v08a`**, one vote each, β = 0.10; placeholder green (four members blended, rc 0, 82 s) | gold-58 of the new members 0.8768 / 0.8816 | ⏳ scoring ≈ 6–10 h | read vs #13 (same β, two members): ≥ +0.005 = the P-28 members earn their place (✅ P-28 on the LB); within ±0.004 🔁; a drop = they are redundant with the stack |
| 2026-09-22 | **Anchor-only control, submission #15 (`rsna-knee-fork` v6, sent 10:08, ref 56461317)**: the public 0.942 graph verbatim, **our arm not run** (β 0 → `status: anchor_control`, `submission.csv` byte-identical to the anchor; same 20 sources as #12/#13) | — | ⏳ scoring ≈ 6 h (no arm) | **the baseline every β read is relative to**: 0.942 = the anchor reproduces from our account (then #12's −0.003 is our arm's); ≠ 0.942 = the anchor itself moved (unpinned sources / rerun variance) and #12–#14 are read against this number |
| 2026-09-22 | **`v09p` (P-29, kernel `rsna-knee-train` v21, pushed 13:05)**: the `v09h` recipe on fold 0 (CoAtNet-1 @224, c02, window_attn) with the production schedule's **16 epochs**, per-epoch OOF csvs | fold-0 OOF-vs-teacher per epoch (871 studies, floor 0.008) | — | ⏳ ≈ 4 h. Read: peak epoch and peak − epoch-15; < 0.008 = no early peak, 16 epochs stays; ≥ 0.008 with the peak before epoch 10 = the P-28 schedule over-trains → epoch budget changes. Also vs `v09h` 8-ep fold 0 (0.8683): does 16 beat 8 at all |
