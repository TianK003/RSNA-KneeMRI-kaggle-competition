...
| 2026-09-26 | **Raptor teacher pass — shard 2/3 (`rsna-knee-teacher` v5, pushed 17:26)**: the v4 render with `SHARD = 2` (the only diff; `teacher_pass_test.py` green), the last 1,449 gold-free studies; merge target `--expect-n 4349` | — | — | ⏳ **QUEUED ≥ 1 h 40 min without starting** (17:26 → 19:07+, empty `failureMessage`); a throwaway T4 probe kernel (`rsna-knee-gpu-probe`, `nvidia-smi -L`) pushed 19:04 queues too → Kaggle T4 capacity, not this kernel (traps 41). Budget 2.5 h once it starts |
