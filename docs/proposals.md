...
| P-13 | 3 vs 5 folds under a fixed session budget [our hypothesis] | 💡 untested, but **directly supported 2026-08-29**: 5 folds alone +0.009 LB, 5 folds on top of a second head **+0.000**, and (2026-09-03, #11) 5 folds of `v09h` inside the seven-vote blend **+0.001** — diversity beats replicates (experiments.md, "First valid 5-fold run", RESOLVED note) | **raised — the next GPU spend is a diverse member, not folds** | 1–2 sessions | P-10 |
...
| P-27 | **Public-stack fork + our arm** — the 0.942 notebook's graph verbatim (cells 0–49) with our c02 members rank-blended at β | ✅/🔁 **measured 2026-09-22**: anchor-only control #15 = **0.942**; our arm ±0.000 at β 0.10 (#13 0.942), −0.003 at β 0.20 (#12 0.939), −0.001 with the P-28 members (#14 0.941) — all sub-floor (experiments.md 2026-09-22 P-27 read-out) | **delivered: 0.913 → 0.942**; our arm's increment ≈ 0 publicly | done | P-23, CC0 licences |
| P-28 | **Production training regime: all 4,349 studies, 16 epochs, SWA of the last 3 EMA snapshots** | ✅ **S2 retrained 2026-09-23 (train v26, 2.64 h both T4s) → experiments.md "S2 production retrain"**: `v09a` (8 ep + S1 knobs) gold-58 SWA **0.8922** (16-ep 0.8768), `v08a` (8 ep) **0.8850** (0.8816) — both still rising at epoch 7 (no peak-and-drift), gold deltas direction-only; shipped as new Dataset versions; **LB #17 = 0.941 🔁** (fork v8, β 0.10: −0.001 vs 0.942, = #14; the same `v09a` alone = 0.918 solo, #18). Earlier 🔁 (2026-09-22): the 16-ep members −0.001 on the LB (#14 vs #13); P-29 → `epochs` 8 | medium — the regime minus its epoch budget | 2 × one T4 session (spent) | P-25/P-26; P-29 |
...
| P-34 | **CoAtNet backbone LR 3e-5 instead of 1e-4** (`v09d` = the `v09c` recipe with the public 0.928 member's backbone LR) | ❌ **HARMFUL 2026-09-23** — `v09d` 0.8596 vs `v09c` 0.8730 (−0.0134, 10/12 down); drops P-36; see experiments.md "Round 2" | **medium** — the P-32 "if it fails" branch; a hybrid's pretrained features may be over-written at 1e-4 (loss falls faster than OOF rises, P-29) | ≈ 3 h T4 (beside `v08c`) | P-32 / P-33 (`v09c` is the control), P-31 |
| P-35 | **Light augmentation on the DINOv2 arm** (`v08c` = the `v08w` recipe + `aug="light"`) | 🔁 **INCONCLUSIVE 2026-09-23** — `v08c` 0.8650 vs `v08w` 0.8648 (+0.0002); see experiments.md "Round 2" | **medium** — P-33 read +0.004 on the CoAtNet (🔁); the public DINO members train with the same rot ±8° / scale / shift / intensity recipe | ≈ 1.5 h T4 (beside `v09d`) | P-33, P-25 / P-26, P-31 |
| P-36 | **CoAtNet at the public schedule: 16 epochs × backbone LR 3e-5** (`v09e` = the `v09c` recipe + `epochs 16`, `lr_backbone 3e-5`) | ❌ **DROPPED 2026-09-23 (not run)** — P-34 read < 0.865; re-openable (≈ 1.7 h) only because `v09d` was still rising | **medium** — P-29's over-training was measured at 1e-4; the public 0.924 member trains 16 epochs at 3e-5, a pair never run together here | ≈ 1.7 h 4090 | P-32 / P-33 (`v09c`), P-34, P-31 / P-24 |
...
Status: 💡 untested as an A/B, but the fold side of the trade is now measured three times and the answer is
consistent: **folds are worth +0.009 LB as a lone version (#6), +0.000 on top of a second head (#7), and
+0.001 inside a seven-vote blend (#11, 2026-09-03 — the 5-fold `v09h` vs the 1-fold `v09h` of #10, both 🔁
under the 0.005 floor)**. So the card's premise holds in the direction it predicted — diversity beats
replicates — and the remaining open half is only whether *3* folds of two families beat 5 of one, which no
submission has isolated. Do not spend a submission on more folds of an existing member.
...
Status (2026-09-03): ✅ **the lane's final number is LB 0.913 — submission #11**, the seven-version blend
with `v09h` as five folds (+0.001 over #10 → 🔁 as pre-registered; folds are replicates inside one vote of
seven). **That blend is the default and the whole c02 lane is now mined out**: three members, ρ ≈ 0.84
between any two, and the third one bought +0.003 while its five folds bought +0.001. The card's acceptance
rule is therefore unchanged and binding for the next candidate — it must be a new **input representation**
(#3 gated-stem 16-ch, #4 RadImageNet behind the licence gate) or P-17 self-training on the multi-family OOF
(`artifacts/kaggle_out/pod_v09h_5fold/`), not another backbone on the same windows. Earlier status follows.
Status (2026-08-30 19:50): ✅ **LB confirms the lane — #9 (six versions: #8's four + `v08w` + `v10c`) = 0.909, #10 (+ `v09h`) = 0.912 = the default blend** (+0.012 over 0.900, 2.4× the 0.005 floor; the `v09h` increment +0.003 is 🔁 as the OOF predicted; offsets +0.0295 / +0.030); **the 5-fold `v09h` is done** (per-fold 0.8683/0.8668/0.8589/0.8546/0.8653, pooled 0.8625 vs `v05g` 0.8467; experiments.md 2026-08-30 "5-fold `v09h`"); the next candidate must be a new *input representation* (#3/#4) or P-17 — the c02 family is mined out (three members, ρ 0.84, +0.003 LB for the third). Earlier status (16:45): ✅ **candidate #2b `v10c` measured** — fold-0 OOF 0.8641 (v08w 0.8648), ρ 0.73–0.81 vs the DINOv2 members, Lateral/Medial Meniscus 0.858/0.922 (best of all), ACL/MCL weakest; 4-member LB blend + v08w + v10c = **0.8795 (+0.0073)**; v08w+v10c pair +0.0088 over v08w alone; still rising at epoch 7 → a 10–12-epoch rerun is a candidate arm; **`v09h` (224 probe) = 0.8683 in 50 min — the best single, 7-member blend 0.8820** (experiments.md 2026-08-30 `v09h`) (experiments.md 2026-08-30 `v10c`). Earlier status (2026-08-30 afternoon): **candidate #2 implemented, not yet run** — `v09h` = `timm:coatnet_rmlp_1_rw_224`
...
Status: ✅/🔁 **2026-09-22 (evening), read-out complete**: control #15 (anchor only) = **0.942**, #13 (β 0.10) = 0.942,
#14 (β 0.10 + P-28 members) = 0.941, #12 (β 0.20) = 0.939 — the anchor reproduces; our arm is worth ≈ 0 publicly and
negative at higher β (sub-floor): the card's "if it fails" branch — GPU goes only to members *different* from the
stack; β 0.10 kept as the default fork (no public cost, one untuned vote). Earlier: 📊 **measured 2026-09-22 — submission #12 (fork v3, β = 0.20, `v08w` + 5-fold `v09h`) = 0.939**, below the
pre-registered 0.940 line: the action rule fires (fork v4 = β 0.10 → **submission #13**, ref 56458837; fork v5 = β 0.10 + the P-28 members `v09a` /
`v08a` → **submission #14**, ref 56459131 — both scoring; fork v6 = **the anchor-only control**, `--beta 0.0`, our arm
not launched, `status: anchor_control` — **submission #15**, ref 56461317, sent 2026-09-22 10:08, scoring); as evidence −0.003 is 0.6×
the LB floor → 🔁 (experiments.md Submissions #12). Built 2026-09-21 — `src/build_fork.py` writes `kaggle/rsna-knee-fork/` from
...
Status: ✅ **S2 retrained 2026-09-23 → experiments.md "S2 production retrain (train v26)"**: `v09a` gold-58 SWA 0.8922 (16-ep 0.8768),
`v08a` 0.8850 (0.8816), 2.64 h wall for both on the two T4s, curves still rising at epoch 7; gold deltas are direction only (floor
0.05). Shipped 11:44 (Datasets re-versioned); fork v8 (β 0.10, `v08w v09h v09a v08a`) → **submission #17 = 0.941 🔁** (read 20:00: −0.001 vs 0.942, identical to #14 with the 16-epoch members; the same `v09a` alone = 0.918, #18 — experiments.md 2026-09-23 "Submission #17 read 0.941"). Was: ⏳ S2
retrain running — `rsna-knee-train` v26 (pushed 09:05 on Tian's go, `PARALLEL_ARMS = ("v09a", "v08a")`, one arm per T4, ≈ 2.8 h): `v09a` = `PROD` (8 ep, SWA 5–7) + the S1 knobs `batch_studies=2, grad_accum=2, aug="light"` (P-32 / P-33, both 🔁
but same-direction), `v08a` = `PROD` unchanged; Kaggle smoke v25 + local smoke green first. Read: gold-58 SWA (direction only), then
the fork at β 0.10 vs 0.942 (≥ 0.947 = the arm counts; expected 🔁). Was: 🔁 **2026-09-22 (evening)**: #14 (with `v09a` + `v08a`) =
0.941 vs #13 0.942 → the two members add nothing on the LB;
...
### P-29 Epoch-budget probe: does the 16-epoch production schedule peak early on OOF?
Status:       ❌ **measured 2026-09-22 → experiments.md "P-29 epoch-budget probe"**: OOF peak 0.8731 at epoch 8, epoch 15 0.8607
(−0.0124, 11/12 labels down); the hypothesis fails, P-28 `epochs` → 8. Was: ⏳ running — arm `v09p` in `ARMS` (`{**C02, "epochs": 16}`, CoAtNet-1 @224, lr_backbone 1e-4,
fold 0 only, `ckpt_policy="best_oof"`), kernel `rsna-knee-train` **v21** (`ARM_ONLY = "v09p"`, `FORCE_SMOKE = False`, pushed
13:05; local smoke + Kaggle smoke v20 green first; Tian's go-ahead for the real push given in advance). Expected ≈ 4 h →
≈ 17:00; read with step 1 of the card's Measure.
Hypothesis:   under the production schedule (OneCycle over 16 epochs, bs 1 × 24 windows) the fold-0 OOF-vs-teacher of the
`v09h` recipe keeps rising or plateaus: the epoch with the best OOF is ≥ 12 **and** the epoch-15 OOF is within 0.008 of
that peak. The gold-58 drift of `v09a` / `v08a` (peak 0.8925 / 0.8944 at epoch 5–6 → 0.8762 / 0.8802 at epoch 15) is then
58-row noise, and 16 epochs stays.
Origin:       our own measurement (experiments.md 2026-09-22, P-28 arms) + the public 0.924 member's script, which selects its
best gold-58 epoch and SWAs the top 3 — behaviour that only pays if the curve really peaks early.
Evidence:     for it: two arms, same sign, ≈ −0.015 over 10 epochs while the loss keeps falling 0.45 → 0.32 (classic
over-fitting of soft targets); `v09h` 5-fold at 8 epochs peaked at epochs 5–7 under `best_oof`. Against: each drift is well
inside the gold SE (≈ 0.04); an 8-epoch OneCycle's epoch 6 is not a 16-epoch OneCycle's epoch 6 (the LR tail differs), so the
8-epoch peaks say nothing about the 16-epoch schedule.
Measure:      `v09p_fold0_ep{0..15}_oof.csv` → macro OOF-vs-teacher on the 871 fold-0 studies per epoch
(`python src/oof_epoch_analysis.py` or a one-liner with `blend_check.py`'s AUC); report the peak epoch, the peak value, the
epoch-15 value and the 8-epoch twin `v09h` fold 0 (0.8683) for the "does 16 beat 8 at all" question.
Noise floor:  0.008 macro OOF (measured, kernel v11). Peak − epoch-15 < 0.008 = no early peak (hypothesis holds);
≥ 0.008 with the peak before epoch 10 = the production schedule over-trains → P-28 changes.
Cost:         ≈ 4 h on one T4 (3,480 training studies × 16 epochs at 0.25 s/study ≈ 14.5 min/epoch), 0 submissions.
If it works:  16 epochs confirmed for P-28; `v09p_fold0_best.pt` (best_oof) is a bonus fold-0 member of the `v09h` family
(not a new vote — same family).
If it fails:  set P-28 `epochs` to the OOF peak epoch (+SWA over the last 3 of *that* budget), or keep 16 with `swa_last`
covering the peak window; retrain `v09a` / `v08a` under the new budget before any more production members.
Depends on:   P-28 (the regime it tests); the c02 cache + CoAtNet-1 weights mounted in `rsna-knee-train`.

### P-30 Public soft labels (`dreaddevelopment/rsna-knee-labels`) as a 4th teacher source
Status:       ❌ **DEAD END 2026-09-22 (as a measurable idea)** — `labels_llm_soft.csv` has **4,349 rows = the report-only
studies, zero gold rows** (its author validated on gold-58 and published only the training labels), so the source cannot be
scored on gold and the 4-source blend is unchanged at 0.8948 (NaN on every gold row). Agreement with hans_v4 on the 4,349:
mean Spearman 0.81 (hans_v4 vs pilkwang: 0.83), systematically fewer positives. Not adopted; the `--sources` flag in
`src/build_targets.py` and the downloaded file stay (experiments.md Label sources 2026-09-22).
Hypothesis:   the dread soft labels alone score ≥ 0.893 gold-58 macro-AUC (hans_v4's level) and the 4-source probability
blend scores ≥ the current teacher's 0.8948; then future arms train on the 4-source teacher.
Origin:       the public 0.924 member (`dreaddevelopment/knee-mri-training-the-twelve-finding-model`) trained on these
labels *alone*; its author reports the corpus, not the label quality, as the lever — so the labels are at least good enough
to reach 0.924 with one CoAtNet-2.
Evidence:     our three sources score 0.893 / 0.870 / 0.835 alone and 0.8948 blended (experiments.md 2026-08-28); adding a
weaker fourth source has been 🔁 before ("Blending more sources", 2026-08-28) — the bar is therefore *parity with hans_v4
alone*, not "any improvement".
Measure:      `build_targets.py`'s gold block: the source's own gold-58 macro-AUC, the 4-source BLEND gold-58 macro-AUC, the
per-label table, and coverage (rows with all 12 labels non-NaN) — printed side by side with the default run.
Noise floor:  gold-58: 0.02 between sources is noise (the file's own note); 0.05 macro for a decision by CLAUDE.md's rule. So:
adopt only if the source alone ≥ 0.893 **and** the blend does not drop below 0.8948 − 0.005; a blend gain alone is not evidence.
Cost:         1 h CPU, 0 GPU, 0 submissions. **Any adoption resets OOF-vs-teacher comparability**: an arm trained on the
4-source teacher must carry a new version and be compared to peers trained on the same teacher (or on gold-58 / the LB).
If it works:  the 4-source teacher becomes the default for *future* arms (P-28 members retrain on it only if P-29 forces a
...
Status:       ✅ **measured 2026-09-23 → experiments.md "S1 A/B on both T4s (train v23)"**: two arms in 3.22 h wall, children at
0.25 / 0.28 s/study (1.00× / 1.12× solo, under the 1.3× line), 6.84 GiB peak each, both `_best.pt`, rc 0; the default from S2 on
(`PARALLEL_ARMS = ("v09a", "v08a")`). Was: ⏳ implemented; first real two-arm session in flight — `rsna-knee-train` v23 (pushed
2026-09-22 20:54, `v09b` ‖ `v09c`, ≈ 3 h); Kaggle smoke v22 green (children on `cuda:0` / `cuda:1`, both `_best.pt`, rc 0, 0.03 h; 2 × 24 windows = 6.84 GiB peak).
Design — `PARALLEL_ARMS = ()` in the config cell (sed'd at build like `ARM_ONLY`); when
...
Status:       🔁 **measured 2026-09-23 → experiments.md "S1 A/B on both T4s (train v23)"**: `v09b` 0.8690 vs `v09h` 0.8683 —
+0.0008 (0.1× the 0.008 floor), 7/12 labels up with seed-scatter signs; the BatchNorm hypothesis is not supported. The knob rides
along in the S2 `v09a` retrain by the same-direction rule (both A/B arms beat `v09h`), at zero cost (0.25 s/study, 6.84 GiB).
Round 2 tests the backbone LR instead (this card's "if it fails"). Was: ⏳ running — `rsna-knee-train` v23, pushed 2026-09-22
20:54 on Tian's go (≈ 3 h; local smoke + Kaggle smoke v22 green first; 2 × 24 windows peak 6.84 GiB on the T4, no
grad-checkpoint needed). Arm `v09b` = the `v09h` recipe (CoAtNet-1 @224, c02, window_attn, 8 ep,
...
Status:       🔁 **measured 2026-09-23 → experiments.md "S1 A/B on both T4s (train v23)"**: `v09c` 0.8730 = +0.0039 over `v09b`
(augmentation alone, 0.5× the floor, 8/12 up; Lateral OA +0.020, Fracture +0.014) and +0.0047 over `v09h`; plateau at epochs 6–7.
Into the S2 `v09a` retrain (same-direction rule); "if it works" round 2 = `aug="light"` on the DINOv2 arm stays a candidate.
Was: ⏳ running — `rsna-knee-train` v23, pushed 2026-09-22 20:54 (≈ 3 h), on `cuda:1` beside `v09b`; local smoke (loss path
exercised, `aug light` vs `none` children diverge) and Kaggle smoke v22 green first. Arm `v09c` = `v09b` +
`aug="light"`, same session (S1).
...
Status:       ❌ **HARMFUL — measured 2026-09-23 → experiments.md "Round 2"**: `v09d` 0.8596 vs `v09c` 0.8730 (−0.0134, 10/12 labels down, Lateral Meniscus −0.059); the hybrid under-trains at 3e-5 in 8 epochs (still rising +0.0008/epoch at 7). Its rule drops P-36 (`v09e`). Card closed.
...
run needs Tian's go.
Hypothesis:   the fold-0 OOF-vs-teacher of the c02 CoAtNet member rises by ≥ 0.008 over `v09c` (0.8730) at a backbone LR of 3e-5,
because at 1e-4 the ImageNet-pretrained hybrid features are over-written faster than the 3,525 soft-labelled studies can
re-teach them (train loss 0.40 at epoch 7 while OOF has plateaued; P-29's over-fit past epoch 8).
Origin:       the public 0.924 / 0.928 CoAtNet member trains OneCycle 3e-5 (backbone) / 1e-3 (head) (experiments.md 2026-09-22
night, "public training recipes"); our 1e-4 came from the P-10 card for an ImageNet-supervised CNN ("tolerates 5× the LR that
DINOv2's SSL features need") and was never A/B'd on the hybrid.
Evidence:     for: it is the last recipe difference to the public CoAtNet not yet measured (batch composition P-32 ≈ 0, augmentation
P-33 +0.004, 16 epochs harmful P-29); the public member reaches 0.928 solo on the LB where ours is ≈ 0.90. Against: LLRD already
puts the lower blocks at 1e-4 × 0.75^k; DINOv2 at 2e-5 vs 1e-4 was chosen, not measured, for the hybrid; one seed each side
(floor 0.008, per label 0.03).
Measure:      `v09d_fold0_oof.csv` (best_oof epoch) → macro OOF-vs-teacher on the 882 fold-0 studies (`s1_readout.py` pattern /
`src/blend_check.py`) vs `v09c` 0.8730; per-label table; the per-epoch curve (does 3e-5 still climb at epoch 7 → a 12-epoch follow-up).
Noise floor:  0.008 macro. **≥ 0.881 = ✅ KEEP; 0.865–0.881 = 🔁; < 0.865 = harmful.** Support: ≥ 9/12 labels up.
Cost:         ≈ 3 h of one T4 (0.28 s/study × 3,525 × 8 + val), in the second GPU session beside `v08c`; 2 config lines; 0 submissions.
If it works:  `lr_backbone=3e-5` joins the CoAtNet PROD recipe (a third `v09a` retrain, ≈ 2.8 h) and the OneCycle shape becomes a card.
If it fails:  the c02 CoAtNet recipe is exhausted as a lever (batch, aug, epochs, LR all ≈ 0) — the member-quality wall is in
the input representation / targets, and GPU goes to P-23 #3 / #4 or P-17, not to more c02 CoAtNet arms.
...
Status:       🔁 **INCONCLUSIVE — measured 2026-09-23 → experiments.md "Round 2"**: `v08c` 0.8650 vs `v08w` 0.8648 (+0.0002, 6/12 up) — light augmentation is worth nothing on the DINOv2 arm; `v08a` stays `aug="none"`. Card closed.
...
the only untested public-recipe difference on this arm is augmentation, and the DINO members of the public stack train with the
same rot ±8° / scale +0–8 % / shift ±5 % / intensity ±10 %.
Origin:       P-33's "if it works" branch (read 🔁 +0.004 on the CoAtNet — same direction as hypothesised, half the floor); the
public DINOsaur trainer cell (experiments.md 2026-09-22 night).
Evidence:     for: P-33's +0.004 with 8/12 labels up; research.md §2.2 (augmentation universal among RSNA winners); the DINOv2 arm
is the cheaper half of every production pair, so a gain is cheap to productionise. Against: one seed; P-33 did not clear the
floor on the CoAtNet; `v08w` was trained 2026-08-30 on a T4 (same seed 42, same code path — a valid but not same-session control).
Measure:      `v08c_fold0_oof.csv` (best_oof epoch) → macro OOF-vs-teacher on the 882 fold-0 studies vs `v08w` 0.8648; per-label table.
Noise floor:  0.008. **≥ 0.873 = ✅ KEEP; 0.857–0.873 = 🔁; < 0.857 = harmful.** Support: ≥ 9/12 labels up.
Cost:         ≈ 1.5 h of one T4 (0.12–0.14 s/study × 3,525 × 8 + val) beside `v09d`; 1 config line; 0 submissions.
If it works:  `aug="light"` joins the DINOv2 PROD recipe (`v08a` retrain) and both production members carry it.
...
Status:       ❌ **DROPPED 2026-09-23 (not run)** by its own pre-registered rule — P-34's `v09d` read 0.8596 < 0.865 (experiments.md "Round 2"). Re-openable for ≈ 1.7 h of Kaggle quota only because `v09d` was still rising at epoch 7; the arm dict stays in `ARMS`.
...
Status:       🔁 **INCONCLUSIVE — measured 2026-09-23 → experiments.md "RunPod arms"**: `v09f` 0.8717 vs `v09c` 0.8730 (−0.0013, 3/12 up). `pos_weight_max` stays 0; the "Rejected without testing" reasoning is confirmed by measurement. Card closed.
...
Status:       ❌ **DEAD END in production (2026-09-24, #19 0.917 vs #18 0.918 — experiments.md "Submission #19")**. Was ✅ on fold 0 — measured 2026-09-23 → experiments.md "RunPod arms": `v09s` 0.8839 vs `v09c` 0.8730 (**+0.0109, 12/12 labels up**, floor 0.008) — the only recipe change since `v09h` that clears the floor. **Production retrain done 2026-09-23 evening → experiments.md "`v09t`"**: the `v09a` recipe on `("selfdistill_v1",)` as `v09t` (RunPod 4090, 35 min), gold-58 SWA **0.9009** vs `v09a` 0.8922 (+0.0087, direction only) → **solo submission #19 = 0.917 (read 2026-09-24 00:07) → ❌ DEAD END by the pre-registered rule** (< 0.918; −0.001 vs #18, sub-floor): the fold-0 +0.011 was the student agreeing with the LLM teacher it was distilled from, not a move toward truth — a target-source change is read by gold-58 direction + solo LB only (traps 39). `v09a` stays the production member; `v08t` and the distilled fork are dropped. The Raptor table (P-39, a teacher with *more* information than ours) is the remaining lever.
...
### P-39 Raptor teacher pass — the public 0.942 notebook's Raptor branch as a teacher over our training studies
Status:       🔧 **builder + kernel green, spike done 2026-09-23 → experiments.md "Raptor teacher pass"**: `src/build_teacher_pass.py` → `kaggle/rsna-knee-teacher/` (v1 red on the `train_images` assumption — traps 37; v2 smoke 6/6 in 62 s; **v3 spike 100/100, 0 failed, 5.11 s/study**); `src/merge_teacher.py` merges shards. **Full pass after Saturday's quota reset**: one session (4,349 × 5.1 s ≈ 6.2 h < 8 h guard) or `--shard k --n-shards 2` in both slots (the second via `--slug tiankljucanin/rsna-knee-teacher-b`); a guard-stopped shard resumes in a sibling slug that mounts it (`--kernel-source`). Then `merge_teacher.py … → artifacts/teacher/raptor_teacher.csv` → `kaggle datasets version` of `rsna-knee-teacher-tables` → Task 12. **Lesson from P-38 (2026-09-24):** judge the Raptor-distilled member by gold-58 direction and the solo LB only — a fold-0 OOF gain against the LLM targets measured agreement with the LLM teacher, not truth (#19 0.917 vs 0.918; traps 39). **Pass running 2026-09-24 14:54: shards 0/3 + 1/3** (`rsna-knee-teacher` v4, `rsna-knee-teacher-b` v1, ≈ 2.1 h each; re-sharded 2 → 3 so two shards fit the week's last 6.3 h of quota) → **both green 17:25: 2,900 studies, 0 failed, 5.6–6.2 s/study, Raptor vs the hard LLM teacher macro AUC 0.906 on 2,900 (experiments.md 2026-09-24 "Raptor pass shards 0–1")**; shard 2/3 (1,449 studies, budget 2.5 h) after Saturday's reset → merge (`--expect-n 4349`) → Dataset version → Task 12. **2026-09-26: shard 2/3 pushed 17:26 (`rsna-knee-teacher` v5), queued ≥ 1 h 40 min on Kaggle T4 capacity (traps 41); Task 12 staged** — arm `v09r` (the `v09a` dict, `SHIPPED_ARMS`, `DISTILLED_ARMS` pins it to `("raptor_teacher",)` — traps 40), local smoke on the 2,900-row partial table green, `rsna-knee-train` render = the `v09r` smoke, `rsna-knee-infer` render = the `v09r` solo (+ `rsna-knee-ckpt-v09r`), pod job `scripts/runpod_chain.sh v09r`; `v08r` (the `v08a` dict) waits for a ✅. **Pass complete 23:39** (shard 2/3 = `rsna-knee-teacher` v5: 1,449 / 0 failed / 7.50 s/study / 3.02 h after a 3 h 10 min queue) → merged 4,349 rows, macro 0.9075 vs the LLM teacher, published 23:40 (experiments.md 2026-09-26 "Raptor pass complete"); `v09r` trained on pod `xwgxw1ai93sjow` (4090, EUR-IS-1, 23:42 → 00:30, ≈ $0.76, deleted): **gold-58 SWA 0.9093** vs `v09a` 0.8922 (direction only; 9/12 labels up, Synovitis +0.079) → Dataset `rsna-knee-ckpt-v09r` → `rsna-knee-infer` v17 (solo) → submission #20 ⏳ (experiments.md 2026-09-26 "`v09r`").
The Raptor branch (three CC0 CoAtNet checkpoints, four views — `maxspan-v5` 0.60, `native384dense-v10` 0.10, `maxspan-v5-reverse` 0.10, `native384-v8` 0.20 — 94 eval windows, two GPUs) runs unchanged over chunks of our 4,349 report-labelled studies (gold excluded) by pointing `RSNA_COMP_ROOT` at a chunk whose `test_series` is a symlink into the mounted `train_series/`; raw per-view probabilities are saved and view-weighted-averaged in probability space; the table enters training through `TEACHER_TABLES=("raptor_teacher",)` (quantile-matched per label onto the LLM blend, mixed 0.5; `y__*` evaluation targets unchanged).
In plain words: **Raptor is not our model and we never train it.** It is the strongest single member inside the public 0.942
              notebook — three publicly shared CoAtNet-2 checkpoints (CC0) that read a knee study in four slice samplings
              ("views") and output 12 probabilities; ≈ 0.924 public LB alone, gold-58 0.905–0.917 held out vs our LLM teacher's
              0.8948. The *pass* runs that code verbatim, frozen, over OUR 4,349 report-only training studies and records what
              it predicts for each — a second, independent, better opinion per study (the *teacher table*). The *shards* are
              a plain partition of that work (≈ 5.6–6.2 s per study on two T4s → three chunks of ≈ 1,450, one kernel run
              each; nothing is learned across shards) merged by `merge_teacher.py` into one 4,349-row table. Task 12 then trains
              OUR production member (`v09a` recipe) with targets = 0.5 · LLM report label + 0.5 · quantile-matched Raptor
              probability — knowledge distillation into our student; the goal is a stronger member of ours, read solo vs 0.918.
              Why it can work where self-distillation (P-38) did not: our own OOF carried no information the LLM labels lacked;
              Raptor was trained elsewhere on different targets and disagrees with the LLM on real cases (0.906 agreement on
              2,900 studies). The risk that the student copies Raptor's public-LB overfitting is why the read is gold-58
              direction + one solo LB, never OOF (traps 39).
Hypothesis:   a production member trained on `0.5 · LLM + 0.5 · quantile-matched Raptor` reads ≥ 0.923 solo on the public LB (baseline #18 = the S2 `v09a` alone, 0.918) and its gold-58 rises in direction (≥ 0.8922 + 0.05 is the floor, so direction only), because the Raptor member's own gold-58 is 0.905–0.917 held out vs the LLM teacher's 0.8948, and P-38 has just shown that smoothing the targets with a *weaker* teacher (our own 0.86 OOF) is worth +0.011 fold-0 OOF with 12/12 labels up.
Origin:       spec `docs/superpowers/specs/2026-09-23-member-strength-design.md` §1 (track A); brainstorm 2026-09-22 path C (Raptor-distillation targets), chosen by Tian 2026-09-23 as a *blend* with the LLM teacher, not a replacement; CoAt family children excluded.
Evidence:     for: the 100-study spike — Raptor ranks the hard LLM labels at macro AUC 0.914 (ACL 0.946, Lateral Meniscus 0.972, Contusion 0.965, Fracture 0.949; MCL 0.772 lowest) with a more positive operating point on every rare label (handled by the matching); P-38 ✅. Against: the student may copy the public member's errors and become *more* correlated with the stack it is meant to add to (the fork read decides); MCL is the label both teachers disagree on most.
Measure:      (1) gold-58 SWA of the distilled `v09a` vs 0.8922 (direction only); (2) **solo public LB via `rsna-knee-infer` with `INFER_MEMBERS=["v09a"]` on the `-rt` Dataset vs #18 0.918**; (3) if ✅, the fork at β 0.10 with the distilled members vs 0.942.
Noise floor:  gold 0.05 (direction only); public LB 0.005: **≥ 0.923 ✅ / 0.919–0.922 🔁 / < 0.918 ❌**.
Cost:         spike 0.2 h done; the pass = 3 shards (0 and 1 done 2026-09-24, 4.7 h; shard 2 ≈ 2.5 h after Saturday's reset); retrain ≈ 41 min on a RunPod 4090 (≈ $0.5) or ≈ 2.7 h (both T4s); 1 solo submission (+ 1 fork submission if ✅).
If it works:  retrain `v08a` on the same teacher, rebuild the fork with `--member v09a=…-rt v08a=…-rt`, β 0.10 → the first member of ours that moves 0.942; then `("selfdistill_v1", "raptor_teacher")` together.
If it fails:  🔁 → the LLM teacher was not the binding constraint for this architecture; ❌ → the student copies the public errors; either way the self-distill lane (P-38) remains and the production members stay as shipped.
Depends on:   P-38 (the mixing mechanism, `TEACHER_TABLES`), Tasks 7–8 (`src/build_teacher_pass.py`, `src/merge_teacher.py`, `src/teacher_pass_test.py`), traps 31 (sibling-slug resume) / 37 (`train_series`), P-24 / P-31, Saturday's quota reset.
...
| Evaluating CoAtNet-2 @384 on **all 60 windows** at rerun | 60 × 47.7 GMACs per study ≈ 0.6–1.2 s on a T4 before decode; the notebook's *primary* CoAtNet arm evaluates **`K_EVAL = 62`** windows (42 is only its v4/v9 legacy arms' value; the cell's docstring saying "42 windows" is stale — re-read 2026-08-30 evening) — we cap at `eval_windows=42` in `v10c` for the T4 rerun budget, same value in `oof_eval` | design review 2026-08-30 |
