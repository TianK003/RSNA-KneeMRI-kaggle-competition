...
| P-29 | **Epoch-budget probe for the production regime**: fold-0 twin of `v09h` trained for 16 epochs (`v09p`, same OneCycle stretched to 16) with per-epoch OOF on the 871 held-out studies | ⏳ **running 2026-09-22** (`rsna-knee-train`, ARM_ONLY=v09p) | **high for validity** — decides whether every P-28 member leaves ≈ 0.015 on the table (gold-58 of `v09a`/`v08a` peaked at epoch 5–6 and drifted); the OOF floor 0.008 can answer what gold-58 cannot | ≈ 4 h T4 | P-28 |
...
fold 0 only, `ckpt_policy="best_oof"`), kernel `rsna-knee-train` with `ARM_ONLY = "v09p"`; smoke first, then the real run
(Tian's go-ahead given in advance, 2026-09-22 morning).
