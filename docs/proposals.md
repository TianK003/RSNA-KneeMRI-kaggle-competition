...
| P-12 | Slice-window TTA (label-safe only) [our hypothesis] | 🔧 **implemented 2026-08-30**; ⏳ **measuring on the RunPod pod** (Kaggle `oof_eval` v2 died OOM on member 2, traps 28; first number v05a 0.8574 → 0.8621 mean-TTA, 🔁 alone — experiments.md); mean and focal both run on the pod before `v10c` · **the submitted 0.936 DINO frontier is the NO-jitter view** (cell re-read), so only the window/offset part is copied | low-medium — cheapest item on the list | 0.5 h GPU to measure | P-01 |
...
Status (2026-08-30 16:45): ✅ **candidate #2b `v10c` measured** — fold-0 OOF 0.8641 (v08w 0.8648), ρ 0.73–0.81 vs the DINOv2 members, Lateral/Medial Meniscus 0.858/0.922 (best of all), ACL/MCL weakest; 4-member LB blend + v08w + v10c = **0.8795 (+0.0073)**; v08w+v10c pair +0.0088 over v08w alone; still rising at epoch 7 → a 10–12-epoch rerun is the next arm; `v09h` (224 probe) training (experiments.md 2026-08-30 `v10c`). Earlier status (2026-08-30 afternoon): **candidate #2 implemented, not yet run** — `v09h` = `timm:coatnet_rmlp_1_rw_224`
