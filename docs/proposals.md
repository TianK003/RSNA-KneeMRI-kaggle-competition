...
pre-registered 0.940 line: the action rule fires (fork v4 = β 0.10 built 2026-09-22; an anchor-only β = 0
submission is the missing control, since the anchor's 0.942 was never scored from our account and the arm is
fail-soft); as evidence −0.003 is 0.6× the LB floor → 🔁 (experiments.md Submissions #12). The P-28 members join
via `--members v08w v09h v09a v08a` (fork v5). Built 2026-09-21 — `src/build_fork.py` writes `kaggle/rsna-knee-fork/` from
