...
| P-27 | **Public-stack fork + our arm** — the 0.942 notebook's graph verbatim (cells 0–49: 20 DINOv2, A5, RadImageNet + calibrator, Raptor ×4 @94 windows, CoAt family, per-label outer weights) with the unattached FineSpacing stage replaced by our c02 members rank-blended at β = 0.20 | 📊 **measured 2026-09-22: submission #12 (β 0.20) = 0.939** — below the 0.940 line, so the action rule fires (#13 = β 0.10 and #14 = β 0.10 + P-28 members scoring; fork v6 = anchor-only control β 0 sent as #15), but −0.003 vs 0.942 is 🔁 as evidence (experiments.md Submissions #12) | **very high — 0.913 → 0.939 in one submission**; our arm's increment is still unmeasured (anchor never scored from our account) | ~1.5 h engineering, ≈ 80 min of hidden-test runtime, 1 submission per β | P-23 (decided its zero-training alternative), the CC0 licences (read 2026-09-21) |
| P-28 | **Production training regime: all 4,349 studies, 16 epochs, SWA of the last 3 EMA snapshots** (`train_all`, `swa_last`, `ARM_ONLY`; arms `v09a` CoAtNet-1 @224 c02 and `v08a` DINOv2-S c02) | ✅ **both arms trained 2026-09-22** (`v09a` 5.1 h gold-58 SWA 0.8768, `v08a` 2.4 h 0.8816; SWA ≈ last EMA 🔁; gold peaks at epoch 5–6 — open question); LB via fork v5 ⏳ (experiments.md 2026-09-22) | **high** — the public 0.924 member's regime (their corpus growth 3,155 → 4,349 alone was +0.010 LB); no OOF for such members, gold-58 + LB only | 2 × one T4 session (v08a ≈ 2.6 h, v09a 4–8 h) | P-25/P-26 recipe; P-27 (the blend it joins) |
| P-29 | **Epoch-budget probe for the production regime**: fold-0 twin of `v09h` trained for 16 epochs (`v09p`, same OneCycle stretched to 16) with per-epoch OOF on the 871 held-out studies | ⏳ **running 2026-09-22** (`rsna-knee-train` **v21**, ARM_ONLY=v09p, pushed 13:05 after smoke v20 green; ≈ 4 h) | **high for validity** — decides whether every P-28 member leaves ≈ 0.015 on the table (gold-58 of `v09a`/`v08a` peaked at epoch 5–6 and drifted); the OOF floor 0.008 can answer what gold-58 cannot | ≈ 4 h T4 | P-28 |
...
Status: 📊 **measured 2026-09-22 — submission #12 (fork v3, β = 0.20, `v08w` + 5-fold `v09h`) = 0.939**, below the
...
Status: ✅ **both arms trained 2026-09-22** (`rsna-knee-train` v19 = `v09a` 5.12 h, gold-58 SWA 0.8768; `rsna-knee-folds`
...
Status:       ⏳ **running 2026-09-22** — arm `v09p` in `ARMS` (`{**C02, "epochs": 16}`, CoAtNet-1 @224, lr_backbone 1e-4,
