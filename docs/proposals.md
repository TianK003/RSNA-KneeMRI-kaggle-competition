...
| P-38 | **Self-distillation targets** (`v09s` = the `v09c` recipe on `TEACHER_TABLES=("selfdistill_v1",)`, mix 0.5: our own 5-fold OOF, quantile-matched onto the LLM blend) | ✅ **KEEP 2026-09-23** — `v09s` **0.8839** vs `v09c` 0.8730 (+0.0109, 12/12 up), RunPod 4090 34 min; see experiments.md "RunPod arms". **Production `v09t` trained 2026-09-23 evening (4090, 35 min): gold-58 SWA 0.9009 vs `v09a` 0.8922 → solo #19 ⏳** (vs 0.918: ≥ 0.923 ✅ / 0.919–0.922 🔁 / < 0.918 ❌) | **medium** — P-17's round-2 targets, never run; the cheap rehearsal of the Raptor-teacher path (P-39) on the same code | ≈ 1 h 4090 | P-32 / P-33 (`v09c`), P-17, P-31 / P-24 |
...
Status:       ✅ **KEEP — measured 2026-09-23 → experiments.md "RunPod arms"**: `v09s` 0.8839 vs `v09c` 0.8730 (**+0.0109, 12/12 labels up**, floor 0.008) — the only recipe change since `v09h` that clears the floor. **Production retrain done 2026-09-23 evening → experiments.md "`v09t`"**: the `v09a` recipe on `("selfdistill_v1",)` as `v09t` (RunPod 4090, 35 min), gold-58 SWA **0.9009** vs `v09a` 0.8922 (+0.0087, direction only) → **solo submission #19 ⏳** (`rsna-knee-infer` v16, ref 56504077; vs #18 0.918: ≥ 0.923 ✅ / 0.919–0.922 🔁 / < 0.918 ❌). Then the Raptor table as the second teacher (P-39).
